...
var objects = []; // lista wszystkich obiektów CSS3D (elementów)
var targets = {
    table: [],
    sphere: [],
    helix: [],
...
var tweenGroup = new tween_js_1.Group(); // grupa tweenów do animacji
function init() {
    ...
    var _loop_1 = function (i, index) {
        ...
        mainContainer.style.backgroundColor = "rgba(0,127,127,".concat(Math.random() * 0.5 + 0.25, ")");
        ...
        var communicationArea = document.createElement('div');
        communicationArea.className = 'communicationArea';
        var messageParagraph = document.createElement('p');
        messageParagraph.textContent = 'Witaj! Odkryj pola, aby zobaczyć, co się pod nimi kryje!';
        ...
        var squareScratch = document.createElement('div');
        squareScratch.className = 'squareScratch';
        for (var j = 0; j < 9; j++) {
            var scratchContainer = document.createElement('div');
        ...
        details.innerHTML = "".concat(table[i + 1], "<br>").concat(table[i + 2]);
        ...
        var objectCSS = new CSS3DRenderer_1.CSS3DObject(mainContainer);
    ...
    for (var i = 0, index = 0; i < table.length; i += 5, index++) {
        _loop_1(i, index);
    }
    ...
    renderer = new CSS3DRenderer_1.CSS3DRenderer();
    ...
    if (tableButton)
        tableButton.addEventListener('click', function () { return transform(targets.table, 2000); });
    if (sphereButton)
        sphereButton.addEventListener('click', function () { return transform(targets.sphere, 2000); });
    if (helixButton)
        helixButton.addEventListener('click', function () { return transform(targets.helix, 2000); });
    if (gridButton)
        gridButton.addEventListener('click', function () { return transform(targets.grid, 2000); });
...
function transform(targetArray, duration) {
    ...
    for (var i = 0; i < objects.length; i++) {
        ...
        var tweenPos = new tween_js_1.Tween(object.position)
            .to({
            x: target.position.x,
            y: target.position.y,
            z: target.position.z
        ...
        var tweenRot = new tween_js_1.Tween(object.rotation)
            .to({
            x: target.rotation.x,
            y: target.rotation.y,
            z: target.rotation.z
    ...
    var tweenUpdate = new tween_js_1.Tween({}).to({}, duration * 2)
        .onUpdate(function () { return render(); });
    tweenGroup.add(tweenUpdate);
    tweenUpdate.start();
}
function moveToCenter(object) {
    var mainContainer = object.element;
    mainContainer.classList.add('active');
    var targetPosition = object.position.clone();
    ...
    var cameraPosition = targetPosition.clone().addScaledVector(normal, 200);
    // ✅ Określamy "pion"
    camera.up.set(0, 1, 0); // pion to oś Y
    // 🔄 Animacja pozycji kamery
    new tween_js_1.Tween(camera.position, tweenGroup)
        .to({
        x: cameraPosition.x,
        y: cameraPosition.y,
        z: cameraPosition.z
    ...
    new tween_js_1.Tween(controls.target, tweenGroup)
        .to({
        x: targetPosition.x,
        y: targetPosition.y,
        z: targetPosition.z
    ...
    for (var _i = 0, objects_1 = objects; _i < objects_1.length; _i++) {
        ...
        if (btn)
            btn.style.display = obj === object ? 'block' : 'none';
    }
...
(_a = document.getElementById('resetBtn')) === null || _a === void 0 ? void 0 : _a.addEventListener('click', resetScene);
function moveToCenter_easy(object) {
    new tween_js_1.Tween(camera.position, tweenGroup)
